%
%function []=ripnoiseb(filename,f1,f2,fRD,fFM,MaxRD,MaxFM,App,M,Fs,NS,NB,Axis,Block,DF,AmpDist)
%
%	
%	FILE NAME 	: RIP NOISE B
...
if Block=='y'
	for k=1:NB
		for l=1:NB
			...
			+ MaxRD/NB*noiseunif(fRD,Fsn,Mnfft,count);
			count=count+1;
		end
	end
else
	for k=1:NB
		RD(count,:)=MaxRD*noiseunif(fRD,Fsn,Mnfft,count);
		count=count+1;
	end
end
...
if Block=='y'
	for k=1:NB
		for l=1:NB
			FM1=noiseunif(fFM/2,Fsn,Mnfft,count+NB^2+1);
