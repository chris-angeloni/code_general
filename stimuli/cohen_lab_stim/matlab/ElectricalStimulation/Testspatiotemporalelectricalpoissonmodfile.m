%
%function []=spatiotemporalelectricalpoissonmodfile(FileHeader,Lambda,fm,Trefractory,PW,PulseType,ChArray,Fs,M,MaxAmp,MaxdB,AmpDist,NB,seed)
%
...
%
function []=Testspatiotemporalelectricalpoissonmodfile(FileHeader,Lambda,fm,Trefractory,PW,PulseType,ChArray,Fs,M,MaxAmp,MaxdB,AmpDist,NB,seed)
...
T=M/Fs;
Et=noiseunifh(0,fm,Fs,M,seed+length(ChArray));
if 1/Lambda*1000==Trefractory
    dN=round(Trefractory/1000*Fs);
    spet=(0:dN:M);
else
    spet=poissongen(Lambda*ones(1,M),Fs,Fs,Trefractory,seed);
end
...
for k=1:length(ChArray)
    ...
    if ChArray(k)==1
        i=find(spet>(k-1)*NB/2+1 & spet<=(k)*NB/2)
