%
%function [ClassData] = bayesextractfeature(ClassData,L)
%
...
%
function [ClassData] = bayesextractfeature(ClassData)
...
for j=1:length(ClassData)
    ...
    for k=1:length(List)
        ...
        ClassData(j).F1m=[ClassData(j).F1m cm];     %Feature 1 - model - mean spl
        ClassData(j).F1v=[ClassData(j).F1v cv];     %Feature 1 - validation
        ...
        ClassData(j).F2m=[ClassData(j).F2m em];     %Feature 2 - model - contrast standard dev      
        ClassData(j).F2v=[ClassData(j).F2v ev];     %Feature 2 - validation
