function [snr, sp]=histogram (other)
...
st=floor(st);
if duration>300, duration=298;, end % This makes histogram run faster.
for i=1:duration
	eval (['histo(' num2str(i) ') = sum(sum(full(t' num2str(i) '(amrow:15, acol:bcol))));'])
end

confactor=(15-amrow+1)*(bcol-acol+1);
if size(histo, 2)>298  %Shorten histo to make it quicker to process.
	histo=histo(1, 1:298);
end
