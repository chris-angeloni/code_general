%
%function [X1,X2]=vripplebinsqrsqr(outfile,f1,f2,Fm1,Fm2,FM,RD,M,Fs,NS,RP,BP,alpha,EnvSeed,CarrSeed,calib)
%
...
%   EnvSeed : Random number generator initial seed for Envelope
%             (Default == 0)
%   CarrSeed: Random number generator initial seed for carriers.
%             (Default== random initial carrier seed)
%	calib	: Speaker calibration data structure (Optional)
%
%RETURNED VALUES
%
%   X1,X2   : Left and right sounds
%
%   (C) Monty A. Escabi, Edit Nov 2009
%
function [X1,X2]=vripplebinsqrsqr(outfile,f1,f2,Fm1,Fm2,FM,RD,M,Fs,NS,RP,BP,alpha,EnvSeed,CarrSeed,calib)
...
for k=1:NS
	Corr(k,:)=round(.5+.5*sin(2*pi*RD*X(k)+2*pi*FM*(1:MEnv)/FsEnv+RP));
end
...
for k=1:NS
	...
	Env1=noisesquare(Fm1,Fm2,FsEnv,MEnv,EnvSeed+k);
	P=(1-Corr(k,:))/2;
...
%pause
	...
	for l=1:length(index1)
		...
		X1(N1+1:N1+NW)=X1(N1+1:N1+NW)+Hinv(k)*W.*sin(2*pi*faxis(k)*(1:NW)/Fs+P);

	end

	%Randomizing Interaural / Binaural Phase for Ipsi Sound
	if BP=='R'
		P=rand*2*pi+P;
	end
	...
	for l=1:length(index2)
		...
		%Contra and Ipsi Sounds
		X2(N2+1:N2+NW)=X2(N2+1:N2+NW)+Hinv(k)*W.*sin(2*pi*faxis(k)*(1:NW)/Fs+P);
