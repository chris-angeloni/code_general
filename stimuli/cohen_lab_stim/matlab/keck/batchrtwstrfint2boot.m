%
%function []=batchrtwstrfint2boot(SpecFile,T1,T2,tapenum,filenum,SPL,MDB,Sound,ModType,UFMR,UFRN,SModType,NBoot)
%
%       FILE NAME       : BATCH RTWSTRF INT 2 BOOT
		...
		if ~isempty( findstr('ch',SpetFile) ) & ~isempty( findstr(fnum,SpetFile) ) & exist(TrigFile) & exist(SpetFile) & findstr(SpetFile,Lst(l,:))
			...
			for m=0:Nspet-1
				...
				if strcmp(SModType,'dB')
					if strcmp(Sound,'MR')
						[taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrfdbintboot(SpecFile,T1,T2,spet,TrigA,Fss,SPL(k),MDB(k),ModType,Sound,200,UFMR,'float',NBoot);
						[taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrfdbintboot(SpecFile,T1,T2,spet,TrigB,Fss,SPL(k),MDB(k),ModType,Sound,200,UFMR,'float',NBoot);
					else
						[taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrfdbintboot(SpecFile,T1,T2,spet,TrigA,Fss,SPL(k),MDB(k),ModType,Sound,200,UFRN,'float',NBoot);
						[taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrfdbintboot(SpecFile,T1,T2,spet,TrigB,Fss,SPL(k),MDB(k),ModType,Sound,200,UFRN,'float',NBoot);
					end
				else
					if strcmp(Sound,'MR')
						[taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrflinintboot(SpecFile,T1,T2,spet,TrigA,Fss,SPL(k),MDB(k),ModType,Sound,200,UFMR,'float',NBoot);
						[taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrflinintboot(SpecFile,T1,T2,spet,TrigB,Fss,SPL(k),MDB(k),ModType,Sound,200,UFMR,'float',NBoot);
					else
						[taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrflinintboot(SpecFile,T1,T2,spet,TrigA,Fss,SPL(k),MDB(k),ModType,Sound,200,UFRN,'float',NBoot);
						[taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrflinintboot(SpecFile,T1,T2,spet,TrigB,Fss,SPL(k),MDB(k),ModType,Sound,200,UFRN,'float',NBoot);
				...
				f=['save ' outfile ' taxis faxis PP Wo1 Wo2 No1 No2 p STRF1A STRF1B STRF2A STRF2B Wo1A Wo1B Wo2A Wo2B No1A No1B No2A No2B ModType Sound MdB SModType SPLN '];
				eval(f);
				disp(['saving ' outfile])
			end
