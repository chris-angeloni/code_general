...
if length(Xc)==1	%If Xc contains only one value
	...
	for k=2:length(Trig)-1
		...
		if ~isempty(index2)
			...
			%Finding Histograms Distributions
			RDHist2=[RDHist2 RDrk(1+floor(spetk2/Fss*Fsn/32*31))];
			FMHist2=[FMHist2 FMrk(1+floor(spetk2/Fss*Fsn/32*31))];
	
			%Finding Time of Spike
			Time2=[Time2 spet2(index2)-Trig(1)];
		end
