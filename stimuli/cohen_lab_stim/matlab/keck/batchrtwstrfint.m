%
%function []=batchrtwstrfint(SpecFile,T1,T2,tapenum,filenum,SPL,MDB,Sound,ModType,UFMR,UFRN,SModType,p)
%
		...
		if ~isempty( findstr('ch',SpetFile) ) & ~isempty( findstr(fnum,SpetFile) ) & exist(TrigFile) & exist(SpetFile) & findstr(SpetFile,Lst(l,:))
			...
			for m=0:Nspet-1
				...
				if strcmp(SModType,'dB')
					if strcmp(Sound,'MR')
						[taxis,faxis,STRF1,STRF2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrfdbint(SpecFile,T1,T2,spet,Trig,Fss,SPL(k),MDB(k),ModType,Sound,200,UFMR);
					else
						[taxis,faxis,STRF1,STRF2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrfdbint(SpecFile,T1,T2,spet,Trig,Fss,SPL(k),MDB(k),ModType,Sound,200,UFRN);
					end
				else
					if strcmp(Sound,'MR')
						[taxis,faxis,STRF1,STRF2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrflinint(SpecFile,T1,T2,spet,Trig,Fss,SPL(k),MDB(k),ModType,Sound,200,UFMR);
					else
						[taxis,faxis,STRF1,STRF2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrflinint(SpecFile,T1,T2,spet,Trig,Fss,SPL(k),MDB(k),ModType,Sound,200,UFRN);
