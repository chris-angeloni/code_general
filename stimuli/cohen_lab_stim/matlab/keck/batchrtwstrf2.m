%
%function []=batchrtwstrf2(SpecFile,T1,T2,tapenum,filenum,SPL,MDB,Sound,ModType,SModType,p)
%
%       FILE NAME       : BATCH RTWSTRF 2
		...
		if ~isempty( findstr('ch',SpetFile) ) & ~isempty( findstr(fnum,SpetFile) ) & exist(TrigFile) & exist(SpetFile) & findstr(SpetFile,Lst(l).name)
			...
			for m=0:Nspet-1
				...
				if strcmp(SModType,'dB')
					[taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrfdb(SpecFile,T1,T2,spet,TrigA,Fss,SPL(k),MDB(k),ModType,Sound,200);
					[taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrfdb(SpecFile,T1,T2,spet,TrigB,Fss,SPL(k),MDB(k),ModType,Sound,200);
				else
					[taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrflin(SpecFile,T1,T2,spet,TrigA,Fss,SPL(k),MDB(k),ModType,Sound,200);
					[taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrflin(SpecFile,T1,T2,spet,TrigB,Fss,SPL(k),MDB(k),ModType,Sound,200);
				end
