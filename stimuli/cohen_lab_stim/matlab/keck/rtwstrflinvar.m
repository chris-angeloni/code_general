...
%
function [p1,p2,p1e,p2e,p1i,p2i,spindex1,spindex2]=rtwstrflinvar(STRF1,STRF2,SpecFile,T1,T2,spet,Trig,Fss,SPL,MdB,Sound,NBlocks,sprtype)
...
Env1=zeros(NF,1+N2);                    %Envelope Buffer for channel 1
...
%Loading Data and Computing CorrCoef
while ~feof(fid) & TrigCount<length(Trig)
	...
	for k=1:length(spettrig1)
		...
		if L < N2
			Env1=( 1 + [S1(:,M-(N2-L-1):M) S2(:,1:L+N1)] - MeanP ) / RMSP*sqrt(PP);
		elseif L+N1 > M
			Env1=( 1 + [S2(:,L-N2+1:M) S3(:,1:N1-M+L)] - MeanP )/RMSP*sqrt(PP);
		else
			Env1=( 1 + S2(:,L-N2+1:L+N1) - MeanP )/RMSP*sqrt(PP);
		end
	...
	for k=1:length(spettrig2)
		...
		%Averaging Pre-Event Spectral Profiles
		if L < N1
			Env2=( 1 + [S1(:,M-(N1-L-1):M) S2(:,1:L+N2)] - MeanP ) / RMSP*sqrt(PP);
		elseif L+N2 > M
			Env2=( 1 + [S2(:,L-N1+1:M) S3(:,1:N2-M+L)] - MeanP ) / RMSP*sqrt(PP);
		else
			Env2=( 1 + S2(:,L-N1+1:L+N2) - MeanP ) / RMSP*sqrt(PP);
		end
	...
	if TrigCount/NBlocks==round(TrigCount/NBlocks)
		...
		if length(p1)>1
			subplot(211)
			hist(p1,-1:.1:1)
			xlabel('Coreelation Coefficient')
			ylabel('Counts')
			pause(0)
		end
