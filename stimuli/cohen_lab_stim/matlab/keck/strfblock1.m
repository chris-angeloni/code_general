%
%function [taxis,faxis,STRF1,No1]=strfblock1(SpecFile,T1,T2,spet,Trig,TrigBlock,Fss,MdB,Sound,UF,sprtype)
%
%       FILE NAME       : STRF BLOCK 1
%       DESCRIPTION     : Computes the Contra STRF for a Triggered Block Segment
%
...
%	STRF1		: Spectro-Temporal Receptive Field
%	No1		: Number of Spikes
%
function [taxis,faxis,STRF1,No1]=strfblock1(SpecFile,T1,T2,spet,Trig,TrigBlock,Fss,MdB,Sound,UF,sprtype)
...
fseek(fid,NT*NF*4*(TrigBlock-2),-1);
...
for k=1:length(spettrig1)
	...
	if L < N2
		STRF1(:,:,kk1)=...
		STRF1(:,:,kk1) + [S1(:,M-(N2-L-1):M) S2(:,1:L+N1)] - RMSP;
	elseif L+N1 > M
		STRF1(:,:,kk1)=...
		STRF1(:,:,kk1) + [S2(:,L-N2+1:M) S3(:,1:N1-M+L)] - RMSP;
	else
		STRF1(:,:,kk1)=...
		STRF1(:,:,kk1) + [S2(:,L-N2+1:L+N1)] - RMSP;
	end
