%
%function []=batchcorrvar2(BatchFile,Header,MRSpecFile,RNSpecFile,T1,T2,DF)
%
%	FILE NAME       : BATCH CORR VAR 2 
		...
		for l=1:length(UnitsMR)
			...
			if ~strcmp(UnitsMR(l),'+') & ~strcmp(UnitsMR(l),'-')
				...
				if exist(fileMRLin)
					...
					spetr=poissongen(20*ones(1,1200),1,Fs);

					%Downsampling STRFs
					STRF1=STRF1s(:,1:DF:length(taxis));
					STRF2=STRF2s(:,1:DF:length(taxis));

[p1A,p2A,p1eA,p2eA,p1iA,p2iA,spindex1A,spindex2A]=rtwstrflinvar(STRF1,STRF2,MRSpecFile,T1,T2,spet,TrigA,Fs,45,45,'MR',50,'float');
[p1B,p2B,p1eB,p2eB,p1iB,p2iB,spindex1B,spindex2B]=rtwstrflinvar(STRF1,STRF2,MRSpecFile,T1,T2,spet,TrigB,Fs,45,45,'MR',50,'float');

[p1r,p2r,p1er,p2er,p1ir,p2ir,spindex1r,spindex2r]=rtwstrflinvar(STRF1,STRF2,MRSpecFile,T1,T2,spetr,TrigA,Fs,45,45,'MR',50,'float');

					%Saving Output File
					f=['save ' fileMRLin(1:index-1) 'Var p1A p2A p1eA p2eA p1iA p2iA spindex1A spindex2A p1B p2B p1eB p2eB p1iB p2iB spindex1B spindex2B p1r p2r p1er p2er p1ir p2ir'];
...
[p1A,p2A,p1eA,p2eA,p1iA,p2iA,spindex1A,spindex2A]=rtwstrfdbvar(STRF1,STRF2,MRSpecFile,T1,T2,spet,TrigA,Fs,45,45,'MR',50,'float');
[p1B,p2B,p1eB,p2eB,p1iB,p2iB,spindex1B,spindex2B]=rtwstrfdbvar(STRF1,STRF2,MRSpecFile,T1,T2,spet,TrigB,Fs,45,45,'MR',50,'float');

[p1r,p2r,p1er,p2er,p1ir,p2ir,spindex1r,spindex2r]=rtwstrfdbvar(STRF1,STRF2,MRSpecFile,T1,T2,spetr,TrigA,Fs,45,45,'MR',50,'float');
		
					%Saving Output File
					f=['save ' fileMRdB(1:index-1) 'Var p1A p2A p1eA p2eA p1iA p2iA spindex1A spindex2A p1B p2B p1eB p2eB p1iB p2iB spindex1B spindex2B p1r p2r p1er p2er p1ir p2ir'];
					if ~findstr('5.',version)
						f=[f ' -v4'];
					end
					disp(f)
...
[p1A,p2A,p1eA,p2eA,p1iA,p2iA,spindex1A,spindex2A]=rtwstrfdbvar(STRF1,STRF2,RNSpecFile,T1,T2,spet,TrigA,Fs,45,45,'RN',50,'float');
[p1B,p2B,p1eB,p2eB,p1iB,p2iB,spindex1B,spindex2B]=rtwstrfdbvar(STRF1,STRF2,RNSpecFile,T1,T2,spet,TrigB,Fs,45,45,'RN',50,'float');

[p1r,p2r,p1er,p2er,p1ir,p2ir,spindex1r,spindex2r]=rtwstrfdbvar(STRF1,STRF2,RNSpecFile,T1,T2,spetr,TrigA,Fs,45,45,'RN',50,'float');
