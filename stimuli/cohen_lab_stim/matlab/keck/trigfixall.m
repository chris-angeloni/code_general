%
%function []=trigfixall(Header,BatchFile,Fs,NdoubleMR,NtrigMR,NdoubleRN,NtrigRN)
%
%       FILE NAME       : TRIG FIX ALL
%       DESCRIPTION     : Batch file to fix triggers for all analysis types
%
...
for l=1:length(returnindex)-1
	...
	%Generating Input Triggger Filename
	TapeNum=str2num(Param(1,:));
	FileNum=str2num(Param(2,:));
	if FileNum<10
		trigfile=[Header 't' int2str(TapeNum) '_f0' int2str(FileNum) '_Trig.mat' ];
	else
		trigfile=[Header 't' int2str(TapeNum) '_f' int2str(FileNum) '_Trig.mat' ];
	end

	%Finding and Fixing Triggers - if Param(7,:) == 'SPO' 
	%file will not exist and it won't Find Triggers
	if exist(trigfile) & isempty(findstr(Param(7,:),'BAD'))

		%Displaying Output
		clc
		disp(['Loading Trigeers for : ' trigfile])
