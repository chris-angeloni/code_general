%
%function [T,Y,Y1,Y2,Trig]=strfsprpreinttrig(sprfile,timeaxis,freqaxis,STRF1,STRF2,MdB,L,UF,ftype)
%
...
%                 Interpolates STRF and SPR file prior to computing output.
%
...
%                 for MR, L=1706;
%                 for RN, L=1500;
%   UF          : Upsampling factor for SPR. Note that STRF is at desired
%                 sampling rate so upsampling is not necessary
...
while ~feof(fid) & (count-1<L)
	...
	[T,Yblock,Yblock1,Yblock2]=strf2pre(Si,timeaxis,freqaxis,STRF1,STRF2,MdB,'n'); 
	Nb1=length(Yblock1);
	Nb2=length(Yblock2);
