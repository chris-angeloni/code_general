%
%function [taxis,faxis,STRF1,STRF2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrflin(
%         SpecFile,T1,T2,spet,Trig,Fss,SPL,MdB,ModType,Sound,NBlocks,sprtype)
%
%       FILE NAME       : RT WSTRF LIN
%       DESCRIPTION     : Real Time spectro-temporal receptive field
%			  Uses Lee/Schetzen Aproach via Specto-Temporal Envelope
%			  For LINEAR Amplitude Sound distributions 
%
...
%
function [taxis,faxis,STRF1,STRF2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrflin(SpecFile,T1,T2,spet,Trig,Fss,SPL,MdB,ModType,Sound,NBlocks,sprtype)
...
if strcmp(ModType,'dB')
	...
	while ~feof(fid) & TrigCount<length(Trig)-1
		...
		for k=1:length(spettrig1)
			...
			if L < N2
				STRF1=STRF1+20*log10( (1-epsilon)*(1+[S1(:,M-(N2-L-1):M) S2(:,1:L+N1)])+epsilon ) - RMSP;
			elseif L+N1 > M
				STRF1=STRF1+20*log10( (1-epsilon)*(1+[S2(:,L-N2+1:M) S3(:,1:N1-M+L)])+epsilon ) - RMSP;
			else
				STRF1=STRF1+20*log10( (1-epsilon)*(1+[S2(:,L-N2+1:L+N1)])+epsilon ) - RMSP;
			end
		...
		for k=1:length(spettrig2)
			...
			%Averaging Pre-Event Spectral Profiles
			if L < N1
				STRF2=STRF2+20*log10( (1-epsilon)*(1+[S1(:,M-(N1-L-1):M) S2(:,1:L+N2)])+epsilon ) - RMSP;
			elseif L+N2 > M
				STRF2=STRF2+20*log10( (1-epsilon)*(1+[S2(:,L-N1+1:M) S3(:,1:N2-M+L)])+epsilon ) - RMSP;
			else
				STRF2=STRF2+20*log10( (1-epsilon)*(1+[S2(:,L-N1+1:L+N2)])+epsilon ) - RMSP;
			end
...
elseif strcmp(ModType,'lin')
	...
	while ~feof(fid) & TrigCount<length(Trig)
		...
		for k=1:length(spettrig1)
			...
			%Averaging Pre-Event Spectral Profiles
			if L < N2
				%STRF1=STRF1+ ( ( 1 + [S1(:,M-(N2-L-1):M) S2(:,1:L+N1)] ).^2 - MeanP ) / RMSP*PP;
				STRF1=STRF1+ ( 1 + [S1(:,M-(N2-L-1):M) S2(:,1:L+N1)] - MeanP ) / RMSP*sqrt(PP);
			elseif L+N1 > M
				%STRF1=STRF1+ ( ( 1 + [S2(:,L-N2+1:M) S3(:,1:N1-M+L)] ).^2 - MeanP ) / RMSP*PP;
				STRF1=STRF1+ ( 1 + [S2(:,L-N2+1:M) S3(:,1:N1-M+L)] - MeanP ) / RMSP*sqrt(PP);
			else
				%STRF1=STRF1+ ( ( 1 + S2(:,L-N2+1:L+N1) ).^2 - MeanP ) / RMSP*PP;
				STRF1=STRF1+ ( 1 + S2(:,L-N2+1:L+N1) - MeanP ) / RMSP*sqrt(PP);
			end
		...
		for k=1:length(spettrig2)
			...
			%Averaging Pre-Event Spectral Profiles
			if L < N1
				%STRF2=STRF2+ ( (1 + [S1(:,M-(N1-L-1):M) S2(:,1:L+N2)]).^2 - MeanP ) / RMSP*PP;
				STRF2=STRF2+ ( 1 + [S1(:,M-(N1-L-1):M) S2(:,1:L+N2)] - MeanP ) / RMSP*sqrt(PP);
			elseif L+N2 > M
				%STRF2=STRF2+ ( (1 + [S2(:,L-N1+1:M) S3(:,1:N2-M+L)]).^2 - MeanP ) / RMSP*PP;
				STRF2=STRF2+ ( 1 + [S2(:,L-N1+1:M) S3(:,1:N2-M+L)] - MeanP ) / RMSP*sqrt(PP);
			else
				%STRF2=STRF2+ ( (1 + S2(:,L-N1+1:L+N2) ).^2 - MeanP ) / RMSP*PP;
				STRF2=STRF2+ ( 1 + S2(:,L-N1+1:L+N2) - MeanP ) / RMSP*sqrt(PP);
			end
