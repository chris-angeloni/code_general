%
%function [spet,spetjr,spetspon]=spetaddjitter2(spet,sigma,p,lambdan,refractory,Fs,T)
%
%   FILE NAME   : SPET ADD JITTER 2
...
%
function [spet,spetjr,spetspont]=spetaddjitter2(spet,sigma,p,lambdan,refractory,Fs,T)
...
if p<=1
    X=bernoullirnd(p,1,length(spetj));
    ii=find(X==1);
    spetjr=spetj(ii);
end
