%
%function [fsiMR,fsiRN]=batchfsibin2(BatchFile,Header)
%
%	FILE NAME       : BATCH FSI BIN 2
...
%
function [fsiMR,fsiRN]=batchfsi2(BatchFile,Header)
		...
		for l=1:length(UnitsMR)
			...
			if ~strcmp(UnitsMR(l),'+') & ~strcmp(UnitsMR(l),'-')
				...
				if exist(fileMRLin)
					...
					%Finding FSI
					fsiMR(count,1)=fsibin2(p1A,p1B,p2A,p2B,p1r,p2r,spindex1A,spindex1B,spindex2A,spindex2B);
					fsiMR(count,2)=fsibin2(p1eA,p1eB,p2eA,p2eB,p1er,p2er,spindex1A,spindex1B,spindex2A,spindex2B);
					fsiMR(count,3)=fsibin2(p1iA,p1iB,p2iA,p2iB,p1ir,p2ir,spindex1A,spindex1B,spindex2A,spindex2B);
					fsiMR(count,4)=fsibinind([p1A p1B],[p2A p2B],p1r,p2r);
