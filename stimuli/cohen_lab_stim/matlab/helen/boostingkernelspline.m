%
%function [H] = boostingkernelspline(X,Y,N,fv)
%
...
%   fc          : B-spline upper cutoff frequency (Hz)
%   Ns          : B-Spline order
%   Fs          : Sampling rate (Hz)
%   
% (C) Monty A. Escabi, February 2012
%
function [H] = boostingkernelspline(X,Y,N,fv,fc,Ns,Fs)
...
%        alpha=1
    ...
    in=find(Eno==Minn);
    if Minp<Minn
        H=[H; H+BB(ip,:)*eps];      %Boosting kth itteration
        E(k)=Minp;
        Ev(k)=min(Epv);
    else
        H=[H; H-BB(in,:)*eps];      %Boosting kth itteration
        E(k)=Minn;
        Ev(k)=min(Env);
    end
