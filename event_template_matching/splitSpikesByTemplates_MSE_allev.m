function [block, cellInfo, labels, spikes, events, fs] = splitSpikesByTemplates_MSE_allev(root,template,drift_correct,NLX)
...
% all recorded events
allevI = [ones(size(stimOn))*1; ones(size(stimOff))*-1; ...
          ones(size(laserOn))*2; ones(size(laserOff))*-2];
[allev,allevSortI] = sort([stimOn; stimOff; laserOn; laserOff]);
d = round(diff(allev),3);


i = 12

% all template events
tallI = [ones(size(template(i).times{1})); ...
         ones(size(template(i).times{2}))*2];
[tall,tallSortI] = sort([template(i).times{1}; ...
                    template(i).times{2}]);
td = round(diff(tall),3);


% optimize the lag and number of repeats that minimizes error
maxReps = floor(length(d)/length(td));
maxLags = length(d)-length(td)+1;

MSE = nan(maxReps,maxLags);
for i = 1:maxReps
    
    TD = repmat(td,i,1);

    for j = 1:length(d)-length(TD)+1
        ind = j:j+length(TD)-1;
        MSE(i,j) = mean((d(ind) - TD).^2,'omitnan');
    end
end


MSE = lagAndRepeat(td,d,23212,3)


% sweep the template across the observed events, computing
% MSE each lag
MSE = [];
for ii = 1:length(d)-length(td)+1
    ind = ii:ii+length(td)-1;
    MSE(ii) = mean((d(ind) - td).^2);
end
