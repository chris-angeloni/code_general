function [x_orig,y_orig,mdl,ahat,p,dm] = estNLFR_unweighted(fr,frh,npoints,thresh)
...
try
    ...
    a0 = [mean(y);max(y)/max(x);.05];    %[mean(y);.3;.05]; %[mean(y);max(y)/max(x);.05]; %[mean(y);mean(y)/mean(x);.05];
    [ahat,r,J,cov,mse] = nlinfit(x,y,mdl,a0);
catch ME
    try
        mdl = @(a,x)(a(1) + a(2)*exp(x*a(3)));
        a0 = [mean(y);mean(y)/mean(x);.05];
        [ahat,r,J,cov,mse] = nlinfit(x,y,mdl,a0);
    catch ME
        try
            mdl = @(a,x)(a(1) + a(2)*exp(x*a(3)));
